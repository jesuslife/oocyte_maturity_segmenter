...
def compute_iou(outputs, labels):
    ...
    # Iterates over the tensor channels
    iou_list = []
    for i in range(0, n):
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            for images, masks in bar:
                ...
                val_iou.append(iou)
                bar.set_description("val_loss=%.5f val_iou=%.5f" % (float(np.mean(val_loss)), float(np.mean(val_iou))))
                sleep(0.1)
        ...
        scheduler.step(np.mean(val_loss))  # Apply scheduler
        print()
        sleep(0.1)
...
def main():
    ...
    transform_img = tvt.Compose([tvt.ToPILImage(),
                                 ...
                                 tvt.RandomAffine(degrees=(0, 15),
                                                  translate=(0.0, 0.1),
                                                  interpolation=tvt.InterpolationMode.BILINEAR),
                                 tvt.RandomHorizontalFlip(p=0.2),
                                 tvt.RandomVerticalFlip(p=0.2),
                                 tvt.PILToTensor()])

    # Composes the mask transformations
    transform_mask = tvt.Compose([tvt.ToPILImage(),
    ...
    img_names = glob(os.path.join(DATA_DIR, IMG_DIR, '*.jpg'))
    mask_names = glob(os.path.join(DATA_DIR, MASK_DIR, '*.npy'))
    ...
    if n_img != n_mask:
        sys.exit('Error: inputs and targets have different sizes')

    # Sets the train data size
    train_size = round(n_img * 0.8)
